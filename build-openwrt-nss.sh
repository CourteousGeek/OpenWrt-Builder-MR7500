...
trap 'rc=$?; if [ $rc -ne 0 ]; then cp "${LOG_FILE}" /workspace/build-nss-failure.log; echo ""; echo "=== Build failed — failing targets ==="; grep -E "\*\*\*.*Error" "${LOG_FILE}" | tail -n 20; echo ""; echo "=== Build failed — last 150 lines ==="; tail -n 150 "${LOG_FILE}"; fi; rm -f "${LOG_FILE}"' EXIT
